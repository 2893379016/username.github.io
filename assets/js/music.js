...
function loadCustomSites() {
    const musicSitesList = document.getElementById('musicSitesList');
    const savedSites = JSON.parse(localStorage.getItem('customMusicSites')) || [];
    ...
    savedSites.forEach(site => {
        ...
        musicSitesList.appendChild(siteCard);
    });
}
...
function createSiteCard(name, url, description, icon) {
    ...
    siteCard.href = url;
    siteCard.className = 'music-site-card';
...
function addCustomSite() {
    ...
    const siteDescription = document.getElementById('siteDescription').value.trim() || '自定义音乐网站';
...
function saveCustomSite(site) {
    ...
    // Save to localStorage
    localStorage.setItem('customMusicSites', JSON.stringify(savedSites));
}

// Function to remove site from localStorage
function removeSiteFromStorage(url) {
    let savedSites = JSON.parse(localStorage.getItem('customMusicSites')) || [];
...
document.head.insertAdjacentHTML('beforeend', `
    <style>
        ...
        .music-site-card:hover .delete-site {
            opacity: 1;
        }
